...
//                "Sec-Ch-Ua-Platform" to "\"Android\"",
        ...
        val link = "$mainUrl/${filmInfo?.slug}"
        val duration = filmInfo?.time?.substringBefore(" ")?.toIntOrNull()
        val tags = filmInfo?.genres?.data?.mapNotNull { it.name }
        val year = filmInfo?.year
        val rating = filmInfo?.imdb.toString().toRatingInt()
        ...
        val trailer = "https://www.youtube.com/embed/${filmInfo?.trailer?.original?.id}"
    ...
    data class filmInfo(
        ...
        @JsonProperty("genres") val genres: GenresData? = null,
        @JsonProperty("trailer") val trailer: TrailerInfo? = null,
    )

    data class GenresData(
        @JsonProperty("data") val data: ArrayList<Genres>? = arrayListOf(),
    )
